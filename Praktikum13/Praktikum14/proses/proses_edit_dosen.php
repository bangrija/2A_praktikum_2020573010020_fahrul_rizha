...
if (isset($_POST['edit'])) {
    ...
    if ($nama_gambar == '') {
        ...
        $update = mysqli_query($conn, "UPDATE tb_dosen SET nip='$nip', nama='$nama', jurusan = '$jurusan', prodi='$prodi', alamat='$alamat', tgl_lahir=" . ($tanggal_lahir == NULL ? "NULL" : "'$tanggal_lahir'") . ", tempat_lahir='$tempat_lahir' WHERE nip = '$niplama'");
    } else {
        $ukuran_gambar = $_FILES['gambar']['size'];
        if ($ukuran_gambar < 2044070) {
            $file_tmp = $_FILES['gambar']['tmp_name'];
            $sql = mysqli_query($conn, "SELECT gambar FROM tb_dosen WHERE nip = '$niplama'");
            $update = mysqli_query($conn, "UPDATE tb_dosen SET nip='$nip', nama='$nama', jurusan='$jurusan', prodi='$prodi', alamat='$alamat', tgl_lahir=" . ($tanggal_lahir == NULL ? "NULL" : "'$tanggal_lahir'") . ", tempat_lahir='$tempat_lahir', gambar = '$nama_gambar' WHERE nip = '$niplama'");
        } else {

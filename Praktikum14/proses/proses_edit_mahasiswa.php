...
if (isset($_POST['edit'])) {
    ...
    if ($nama_gambar == '') {
        ...
        $update = mysqli_query($conn, "UPDATE tb_mahasiswa SET nim='$nim', nama='$nama', kelas='$kelas', prodi='$prodi', alamat='$alamat', tgl_lahir=" . ($tanggal_lahir == NULL ? "NULL" : "'$tanggal_lahir'") . ", tempat_lahir='$tempat_lahir' WHERE nim = '$nimlama'");
    } else {
        $ukuran_gambar = $_FILES['gambar']['size'];
        if ($ukuran_gambar < 2044070) {
            $file_tmp = $_FILES['gambar']['tmp_name'];
            $sql = mysqli_query($conn, "SELECT gambar FROM tb_mahasiswa WHERE nim = '$nimlama'");
            $update = mysqli_query($conn, "UPDATE tb_mahasiswa SET nim='$nim', nama='$nama', kelas='$kelas', prodi='$prodi', alamat='$alamat', tgl_lahir=" . ($tanggal_lahir == NULL ? "NULL" : "'$tanggal_lahir'") . ", tempat_lahir='$tempat_lahir', gambar = '$nama_gambar' WHERE nim = '$nimlama'");
        } else {
